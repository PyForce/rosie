...
class Arduino(Thread):
    def __init__(self, port='/dev/ttyACM0', baudrate=9600,
                 revolutionSteps=settings.ENCODER_STEPS, sampleTime=0.05):
    ...
    def read_state(self):
        try:
            ...
            if header == COMMAND_GETSTATE_RESPONSE:
                ...
                pulses1 += self.serialPort.read()
                pulses1 += self.serialPort.read()
                pulses1, = struct.unpack("i", pulses1)

                pulses2 = self.serialPort.read()
                pulses2 += self.serialPort.read()
                pulses2 += self.serialPort.read()
                pulses2 += self.serialPort.read()
                pulses2, = struct.unpack("i", pulses2)

                batteryCharge, = struct.unpack('B', self.serialPort.read())
    ...
    def set_constants(self, kc, ki, kd):
        package = struct.pack("<Bfff", COMMAND_SETPIDPARAM, kc, ki, kd)
        try:
    ...
    def set_speeds(self, speed1, speed2):
        ...
        package = struct.pack("<Bff", COMMAND_SETPOINT, speed1, speed2)
        try:
            self.serialPort.write(package)
        except:
            print("      Error using the serial port")
