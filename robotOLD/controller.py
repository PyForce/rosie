...
from robotOLD.load import SETTINGS as settings
...
#==== import and load the robotOLD control board ====
board = None
if os.path.exists(os.path.join(os.getcwd(), 'robotOLD', 'boards',
                  settings.FILENAME)):
...
class Controller:
    def __init__(self):
        ...
        if global_settings.getboolean('general', 'log'):
            self.log_file = open(os.path.join(os.getcwd(), 'logs', "pos.log"),
                                 'w')
    ...
    def move(self, trace, smooth=True):
        """
        Start the movement of the robotOLD.
    ...
    def _win_handler(self):
        ...
        if self.finished:
            self.action_exec()
            return
        threading.Timer(self.next_call - time.time(), self._win_handler)\
            .start()
        self._timer_handler()
    ...
    def _timer_handler(self):
        ...
        if self.motion == "move":
            ...
            if not settings.PID:
                ...
                set_point1, set_point2 = pid.speeds_regulation(set_point1,
                                                               set_point2,
                                                               delta_encoder_1,
                                                               delta_encoder_2,
                                                               elapsed, 0, 0,
    ...
    def navigation(self, encoder1, encoder2):
        """
        Calculate the current position of the robotOLD.
        ...
        if encoder1 > 2000000000 or encoder1 < -2000000000 or encoder2 >\
                2000000000 or encoder2 < -2000000000:
            self.encoder1 = 0
            self.encoder2 = 0
            self.robot.reset_encoders()
    ...
    def action_exec(self):
        """
        Execute predefined actions for the robotOLD.
