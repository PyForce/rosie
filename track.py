...
def generate_track(x_planning=None, y_planning=None, desired=None,
                   z_planning=None, t_planning=None, constant_k=None,
                   constant_t=None, cubic=False):
    if not (len(x_planning) == len(y_planning)):
        raise InvalidTrackParametersException('X and Y Vector for planning must have same length.')
    ...
    if cubic:
        ...
        dn = constant_t / float(desired)

        n = int(dn)

        intervals = len(x_planning) - 1

        xd_vector = range(n * intervals)
        yd_vector = range(n * intervals)
        zd_vector = range(n * intervals)

        xd_dot_vector = range(n * intervals)
        yd_dot_vector = range(n * intervals)
        zd_dot_vector = range(n * intervals)
        ...
        for j in range(0, intervals):
            ...
            for i in range(0, n):
                s = i / float(n)
                xd_vector[n * j + i] = - (s - 1) * (s - 1) * (s - 1) * x_i + s * s * s * x_f + alpha_x * (s * s) * (
                    s - 1) + beta_x * s * ((s - 1) * (s - 1))
                yd_vector[n * j + i] = - (s - 1) * (s - 1) * (s - 1) * y_i + s * s * s * y_f + alpha_y * (s * s) * (
                    s - 1) + beta_y * s * ((s - 1) * (s - 1))

                xd_dot_vector[n * j + i] = - 3 * (s - 1) * (s - 1) * x_i + 3 * s * s * x_f + alpha_x * (
                    3 * s * s - 2 * s) + beta_x * (3 * s * s - 4 * s + 1)
                yd_dot_vector[n * j + i] = - 3 * (s - 1) * (s - 1) * y_i + 3 * s * s * y_f + alpha_y * (
                    3 * s * s - 2 * s) + beta_y * (3 * s * s - 4 * s + 1)
                ...
                zd_dot_vector[n * j + i] = \
                    (yd_dot_dot * xd_dot_vector[n * j + i] - xd_dot_dot * yd_dot_vector[n * j + i]) \
                    / (xd_dot_vector[n * j + i] * xd_dot_vector[n * j + i]
                       + yd_dot_vector[n * j + i] * yd_dot_vector[n * j + i])

                xd_dot_vector[n * j + i] = xd_dot_vector[n * j + i] / constant_t
                yd_dot_vector[n * j + i] = yd_dot_vector[n * j + i] / constant_t

                zd_dot_vector[n * j + i] = zd_dot_vector[n * j + i] / constant_t

            zd_vector[n * j] = theta_i

            for i in range(0, n - 1):
                zd_vector[n * j + i + 1] = zd_vector[n * j + i] + zd_dot_vector[n * j + i] * desired

        return xd_vector, yd_vector, zd_vector, xd_dot_vector, yd_dot_vector, zd_dot_vector
    ...
    for i in range(0, len(x_planning) - 1):
        ...
        i_points = int(t_planning[i + 1] / desired)
        max_count += int(i_points)

    xd_vector = range(max_count)
    yd_vector = range(max_count)
    zd_vector = range(max_count)

    xd_dot_vector = range(max_count)
    yd_dot_vector = range(max_count)
    zd_dot_vector = range(max_count)
    ...
    for i in range(0, len(x_planning) - 1):
        ...
        xd_dot = delta_x / desired
        yd_dot = delta_y / desired
        theta_i = math.atan2(yd_dot, xd_dot)
        ...
        xd_dot_vector[base_interval_index] = xd_dot
        yd_dot_vector[base_interval_index] = yd_dot
        zd_vector[base_interval_index] = theta_i
        zd_dot_vector[base_interval_index] = (theta_i - theta_p_i) / float(desired)

        theta_p_i = theta_i

        for j in range(1, i_points):
            xd_vector[base_interval_index + j] = xd_vector[base_interval_index + j - 1] + delta_x
            yd_vector[base_interval_index + j] = yd_vector[base_interval_index + j - 1] + delta_y
            zd_vector[base_interval_index + j] = theta_i
            xd_dot_vector[base_interval_index + j] = xd_dot
            yd_dot_vector[base_interval_index + j] = yd_dot
            zd_dot_vector[base_interval_index + j] = 0.0
